...
K_MEM_SLAB_DEFINE(bt_gatt_hids_c_reports_mem,
		  ...
		  CONFIG_BT_GATT_HIDS_REPORTS_MAX,
		  4);
		   ...
		   const struct bt_gatt_attr *rep_chrc)
{
	...
	struct bt_gatt_chrc *chrc_val = bt_gatt_dm_attr_chrc_val(rep_chrc);
	assert(chrc_val); /* This is internal function and it has to
	...
	if (chrc_val->properties & BT_GATT_CHRC_WRITE_WITHOUT_RESP) {
		...
		gatt_desc = bt_gatt_dm_desc_by_uuid(dm, rep_chrc, BT_UUID_GATT_CCC);
		if (!gatt_desc) {
	...
	gatt_desc = bt_gatt_dm_desc_by_uuid(dm, rep_chrc, BT_UUID_HIDS_REPORT_REF);
	if (!gatt_desc) {
				   ...
				   struct bt_gatt_hids_c *hids_c)
{
	...
	if (gatt_chrc && gatt_chrc_kbd) {
		...
		LOG_ERR("Inconsistent boot keyboard configuration. Ignoring.");
	} else {
		/* No keyboard present */
			     ...
			     const void *data, u16_t length)
{
	...
	if (length > UINT8_MAX) {
		LOG_ERR("Data size too big trundicating");
		length = UINT8_MAX;
	}
...
int bt_gatt_hids_c_rep_write_norsp(struct bt_gatt_hids_c *hids_c,
				   struct bt_gatt_hids_c_rep_info *rep,
				   const void *data, u8_t length)
{
	int err;
